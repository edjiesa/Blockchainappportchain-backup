...
docker exec microfab peer lifecycle chaincode package /tmp/portchain-cc.tar.gz --path /chaincode --lang node --label portchain-cc_1.3
...
PKG_ID=$(docker exec -e CORE_PEER_LOCALMSPID=portorgMSP -e CORE_PEER_ADDRESS=portorgpeer-api.127-0-0-1.nip.io:8080 -e CORE_PEER_MSPCONFIGPATH=/opt/microfab/data/admin-portorg microfab peer lifecycle chaincode queryinstalled | grep portchain-cc_1.3 | awk '{print $3}' | tr -d ',')
...
for ORG in portorg customsorg bankorg; do
  ...
  docker exec -e CORE_PEER_LOCALMSPID=${ORG}MSP \
              -e CORE_PEER_ADDRESS=${ORG}peer-api.127-0-0-1.nip.io:8080 \
              -e CORE_PEER_MSPCONFIGPATH=/opt/microfab/data/admin-${ORG} \
              microfab peer lifecycle chaincode approveformyorg -o orderer-api.127-0-0-1.nip.io:8080 --channelID mychannel --name portchain-cc --version 1.0.4 --package-id $PKG_ID --sequence 4 || true
done
...
docker exec -e CORE_PEER_LOCALMSPID=portorgMSP \
            ...
            microfab peer lifecycle chaincode commit -o orderer-api.127-0-0-1.nip.io:8080 --channelID mychannel --name portchain-cc --version 1.0.4 --sequence 4 --peerAddresses portorgpeer-api.127-0-0-1.nip.io:8080 --peerAddresses customsorgpeer-api.127-0-0-1.nip.io:8080 --peerAddresses bankorgpeer-api.127-0-0-1.nip.io:8080
